...
cat games.csv | while IFS=',' read YEAR ROUND WINNER OPPONENT WINNER_G OPPONET_G
...
do
  ...
  if [[ -n $TEAM_ID_W || -n $TEAM_ID_O ]]
    then
      if [[ $YEAR != "year" ]]
      then
        INSERT_GAMES=$($PSQL "INSERT INTO games(year, round, winner_id, opponent_id, winner_goals, opponent_goals) VALUES ($YEAR, '$ROUND', '$TEAM_ID_W', '$TEAM_ID_O', $WINNER_G, $OPPONET_G)")
